# Side-by-side animation: 12 km hindcast IVT vs ERA5, 3-hourly over the run window.
# ERA5 comes from the wide-region cache (viz_dates covers the run window at 3-hourly).
#
...
Nt = length(era5_dates)

metadata(name) = Metadata(name; dataset = single_levels, dates = era5_dates,
                          region = wide_region, dir = era5_datadir)
era5_east_series  = FieldTimeSeries(metadata(:vertically_integrated_eastward_water_vapor_flux);  time_indices_in_memory = Nt)
era5_north_series = FieldTimeSeries(metadata(:vertically_integrated_northward_water_vapor_flux); time_indices_in_memory = Nt)
λᵉ, φᵉ, _ = nodes(era5_east_series[1])
λᵉ = @. ifelse(λᵉ > 180, λᵉ - 360, λᵉ)

function model_ivt(date)
    seconds = Dates.value(date - start_date) / 1000
    n = argmin(abs.(model_times .- seconds))
...
frame = Observable(1)
modelₙ = Observable(model_ivt(era5_dates[1]))
era5ₙ = Observable(era5_ivt(1))
on(frame) do n
    modelₙ[] = model_ivt(era5_dates[n])
    era5ₙ[] = era5_ivt(n)
end

fig = Figure(size = (1450, 520))
title = @lift "IVT — " * Dates.format(era5_dates[$frame], "u d HH:MM") * " UTC"
...
CairoMakie.record(fig, joinpath(root, "ivt_comparison.mp4"), 1:Nt; framerate = 4) do n
    frame[] = n
end
